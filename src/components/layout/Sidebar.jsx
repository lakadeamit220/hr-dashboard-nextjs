...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white/5 backdrop-blur-[2px] border border-slate-300/70 shadow-sm border-r border-slate-300/70 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } flex flex-col`}
      >
        {/* Logo Area */}
        <div className="h-16 flex items-center px-6 border-b border-slate-300/70">
        ...
        {/* User Profile */}
        <div className="p-4 border-t border-slate-300/70">
          <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-slate-50/50 cursor-pointer transition-colors">
            <div className="w-9 h-9 rounded-full bg-slate-100/50 flex items-center justify-center text-slate-600">
              <User size={18} />
            </div>
